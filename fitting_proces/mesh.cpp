...
void get_coef_land(Eigen::MatrixX3f &coef_land, std::string name, float scale = 0.6) {
	puts("get_coef_land");
	FILE *fp;
	...
	coef_land.array() *= scale;
	fclose(fp);
}

const int G_nVerts = 11510;
void get_coef_mesh(Eigen::MatrixX3f &coef_mesh, int &tot_obj_num, std::string name, float scale = 0.6) {
	puts("get_coef_mesh");
	FILE *fp;
	int num = 0;
...
void test_coef_mesh(Mesh_my &mesh_std, Mesh_my &mesh, Eigen::MatrixX3f &land, Eigen::MatrixX3f &coef_mesh, Eigen::MatrixX3f &coef_land, int idx) {
	mesh.num_vtx = mesh_std.num_vtx;
	mesh.vtx.resize(G_nVerts, 3);
	for (int i = 0; i < mesh.num_vtx; i++)
		for (int j = 0; j < 3; j++)
			mesh.vtx(i, j) = coef_mesh(idx*G_nVerts + i, j);

	
	mesh.num_rect = mesh_std.num_rect;
	mesh.rect = mesh_std.rect;

	cal_norm(mesh);
#ifndef test_svd_vec_def

	land.resize(G_land_num,3);
	for (int i = 0; i < G_land_num; i++)
		for (int j = 0; j < 3; j++)
			land(i, j) = coef_land(idx*G_land_num + i, j);
...
void draw_land(Eigen::MatrixX3f &land) {
	...
	for (int i = 0; i < G_land_num; i++) {
		glBegin(GL_POINTS);
		glVertex3f(land(i, 0)*scale, land(i, 1)*scale, land(i, 2)*scale);
		glEnd();
	}
}
