...
#endif // win64
#ifdef linux 
	std::string mesh_name = "shape_0.obj";
#endif // linux 
...
void load_lv(std::string name, DataPoint &temp) {
	...
	std::cout << temp.user << "\n";
	//system("pause");
	temp.land_3d.resize(G_land_num, 3);
	for (int i_v = 0; i_v < G_land_num; i_v++) {
		fread(&temp.land_3d(i_v, 0), sizeof(float), 1, fp);
		fread(&temp.land_3d(i_v, 1), sizeof(float), 1, fp);
		fread(&temp.land_3d(i_v, 2), sizeof(float), 1, fp);
	}


	fread(&temp.center_3d(0), sizeof(float), 1, fp);
	fread(&temp.center_3d(1), sizeof(float), 1, fp);
	fread(&temp.center_3d(2), sizeof(float), 1, fp);
...
void solve(DataPoint &data, Eigen::MatrixXf &bldshps, char suffix[34469][100], std::string name) {
	...
	for (int i_v = 0; i_v < G_nVerts; i_v++) {
		...
		for (int axis = 0; axis < 3; axis++)
			mesh(i_v,axis) = cal_3d_vtx(bldshps, user, exp, i_v, axis);
	}
	...
	center.setZero();
	for (int i_v = 0; i_v < G_land_num; i_v++) {
		bs.row(i_v) = mesh.row(data.land_cor(i_v));
		center += bs.row(i_v);
	}
	std::cout << "bs------------:\n" << bs << "\n";
	...
	mesh = ((data.shape.rot) * mesh.transpose()).transpose();
		...
		while (index < n)
		{
			...
			if (dp->d_type == DT_DIR) {
				...
				if (dp->d_name[len - 1] == 'v' && dp->d_name[len - 2] == 'l'
					&& dp->d_name[len - 3] == 'd'&& dp->d_name[len - 4] == '3') {
					////	
					std::string p = path + "/" + dp->d_name;
					DataPoint data;
					load_lv(p, data);
					solve(data, bldshps, suffix, p.substr(0, p.find(".3dlv")) + "_0norm.obj");
					std::cout << "cnt" << cnt << "\n";
					cnt++;
				}
			}
...
void solve_same_id(DataPoint &data, Eigen::MatrixXf &exp_r_t_all_matrix, std::string suffix[34469], std::string name) {
	...
	for (int i_v = 0; i_v < G_nVerts; i_v++) {
		for (int axis = 0; axis < 3; axis++)
			for (int i_shape = 0; i_shape < G_nShape; i_shape++)
				mesh(i_v,axis) += data.shape.exp(i_shape)*exp_r_t_all_matrix(i_shape, i_v * 3 + axis);		
	}
	Eigen::MatrixX3f bs(G_land_num, 3);
		...
		while (index < n)
		{
			...
			if (dp->d_type == DT_DIR) {
				...
				if (dp->d_name[len - 1] == 'v' && dp->d_name[len - 2] == 'l'
					...
					DataPoint data;
					load_lv(p, data);
					solve_same_id(data, exp_r_t_all_matrix, suffix, p.substr(0, p.find(".3dlv")) + "_0norm.obj");
					std::cout << "cnt" << cnt << "\n";
					cnt++;
				}
			}
