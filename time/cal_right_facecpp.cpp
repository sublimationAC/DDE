...
//		if ((bj[i] == 0) && pts[i].pt2d.y >= pts[x].pt2d.y - range-10 && (fabs(pts[i].pt2d.y - pts[x].pt2d.y) < mi))
...
//	std::sort(pts.begin(), pts.end(), cmp_x);
//	FILE *fp;
//	fopen_s(&fp, "start_pt_idx_right.txt", "r");
//	memset(bj, 0, sizeof(bj));
//	int st;
...
//
//			int nx = -1, mi = 9999;
//			if (slt_line[cnt_line].size()<3)
//				get_y_nearest(x, 8, nx, mi, pts);
//			else
//				if (slt_line[cnt_line].size() < 8)
//					get_y_nearest(x, 40, nx, mi, pts);
//				else
//					if (slt_line[cnt_line].size() < 15)
//						get_y_nearest(x, 80, nx, mi, pts);
//					else
//					get_y_nearest(x, 80, nx, mi, pts);
//			//if (nx == -1 || mi > 8) get_y_nearest(x, 10, nx, mi, pts);
//			/*if (nx == -1 || mi > 8) get_y_nearest(x, 30, nx, mi, pts);
//			if (nx == -1 || mi > 8) get_y_nearest(x, 50, nx, mi, pts);*/
//
//			if (nx == -1 || mi > 8) break;
//			slt_line[cnt_line].push_back(pts[nx]); bj[nx] = 1;
//			x = nx;
...
//	return distance;
//}
//void cal_line_idx_based_on_sym(
//	std::vector <Point_my> *slt_line, int &cnt_line, std::vector <Point_my> &pts_l, std::vector <Point_my> &pts_r) {
//
//	//std::sort(pts.begin(), pts.end(), cmp_x);
//	FILE *fp;
//	fopen_s(&fp, "slt_line_left_scratch.txt", "r");
//	int num;
//	fscanf_s(fp, "%d", &num);
//	memset(bj, 0, sizeof(bj));
//	for (int t = 0; t < num; t++) {
...
//				exit(-2);
//			}
//			int mi_idx = -1; double mi = 99999;
//			for (int k = 0; k < pts_r.size(); k++)
//				if (bj[k] == 0 && dis_cv_pt(pts_l[now].pt2d, pts_r[k].pt2d) < mi) {
//					mi = dis_cv_pt(pts_l[now].pt2d, pts_r[k].pt2d);
//					mi_idx = k;
//				}
//			//bj[mi_idx] = 1;
//			slt_line[cnt_line].push_back(pts_r[mi_idx]);
//		}
//		cnt_line++;
//	}
//
...
//}
//
//void get_left_line(std::vector <Point_my> *slt_line, int &cnt_line, std::vector <Point_my> &pts) {
//	FILE *fp;
//	fopen_s(&fp, "slt_line_left_scratch.txt", "r");
//	int num;
//	fscanf_s(fp, "%d", &num);
...
//				exit(-2);
//			}
//			slt_line[cnt_line].push_back(pts[now]);
//		}
//		cnt_line++;
//	}
//
...
//}
//
//void cal_line_idx_based_on_sym_3d(
//	std::vector <Point_my> *slt_line, int &cnt_line, double mid_x, std::vector <Point_my> &pts) {
//
//	//std::sort(pts.begin(), pts.end(), cmp_x);
//	FILE *fp;
//	fopen_s(&fp, "slt_line_left_scratch.txt", "r");
...
//				double
//					dis = (x[i] - (2 * mid_x - x[idx]))*(x[i] - (2 * mid_x - x[idx])) +
//					(y[i] - y[idx])*(y[i] - y[idx]) +
//					(z[i] - z[idx])*(z[i] - z[idx]);
//				if (dis < mi) mi = dis, mi_idx = i;
//			}
//
//			int now = -1;
//			for (int i = 0; i < pts.size(); i++)
//				if (mi_idx == pts[i].v_idx) {
//					now = i;
//					break;
//				}
//			printf("cal_line_idx_based_on_first %d %d\n", now, cnt_line);
...
//	double mi_l = 99999, ma_l = -999999, mi_r = 999999, ma_r = -999999;
//	for (int i = 0; i < nVerts; i++) {
//		if (z[i] > -0.1) {
//			if (x[i] < mid_x - 0.1) {
...
//					i });
//				mi_l = min(mi_l, pts_lft.back().pt2d.x);
//				ma_l = max(ma_l, pts_lft.back().pt2d.x);
//			}
//		}
//		if (z[i] > -0.15) {
//			if (x[i] > mid_x + 0.09) {
...
//					Point2d(
//						((2 * mid_x - x[i]) - mi_x) / (ma_x - mi_x)*(width - 1),
//						height - 1 - (y[i] - mi_y) / (ma_y - mi_y)*(height - 1)),
//					i });
//				mi_r = min(mi_r, pts_rht.back().pt2d.x);
//				ma_r = max(ma_r, pts_rht.back().pt2d.x);
//			}
//		}
//	}
//
...
//		printf("%d\n", i);
//		pts_rht[i].pt2d.x =ma_r-(ma_r-pts_rht[i].pt2d.x)*(ma_l - mi_l) / (ma_r - mi_r)-7;
//	}
...
//	cal_line_idx_based_on_first_nsty(slt_line, cnt_line, pts_rht);
//	//cal_line_idx_based_on_sym(slt_line, cnt_line, pts_lft,pts_rht);
//	//cal_line_idx_based_on_sym_3d(slt_line, cnt_line, mid_x, pts_rht);
//	//get_left_line(slt_line,cnt_line,pts_lft);
//	
...
//
//
//	fopen_s(&fp, "slt_line_right_scratch.txt", "w");
//
