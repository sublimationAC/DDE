...
void Fern_dde::visualize_feature_cddt(//const Transform &t,
	...
	for (int i = 0; i < features_index_exp.size(); ++i)
	{
		...
		cv::circle(rgb_images, pixel_positions[feature.first], 0.1, cv::Scalar(0, 0, 255), 2);
		cv::circle(rgb_images, pixel_positions[feature.second], 0.1, cv::Scalar(255, 0, 0), 2);
	}



	//cv::Mat images = rgb_images;
