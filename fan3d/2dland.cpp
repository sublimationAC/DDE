	...
	else
	{
		...
		while (index < n)
		{
			...
			if (dp->d_type == DT_DIR)
				...
				if (dp->d_name[len - 1] == 'd' && dp->d_name[len - 2] == '3' && dp->d_name[len - 3] == 'd') {
					////	
					flag = 1;
#ifdef fan3d
					load_3d_land(path + "/" + dp->d_name, ide, id_idx);
#else
...
#ifdef  flap_3dland
					for (int i = G_land_num * (ide[id_idx].num); i < G_land_num*(ide[id_idx].num + 1); i++)
						ide[id_idx].land_3d(i, 1) = temp.rows - ide[id_idx].land_3d(i, 1);
#endif //  flap_2dland
					ide[id_idx].num++;
				}
			}
...
void save_result_one(iden *ide, int i_id, int exp_idx, std::string name) {
	...
	for (int i_v = 0; i_v < G_land_num; i_v++) {
		fwrite(&ide[i_id].land_3d(exp_idx*G_land_num + i_v, 0), sizeof(float), 1, fp);
		fwrite(&ide[i_id].land_3d(exp_idx*G_land_num + i_v, 1), sizeof(float), 1, fp);
		fwrite(&ide[i_id].land_3d(exp_idx*G_land_num + i_v, 2), sizeof(float), 1, fp);
	}

	fwrite(&ide[i_id].center_3d(exp_idx, 0), sizeof(float), 1, fp);
	fwrite(&ide[i_id].center_3d(exp_idx, 1), sizeof(float), 1, fp);
	fwrite(&ide[i_id].center_3d(exp_idx, 2), sizeof(float), 1, fp);
	...
	else
	{
		...
		while (index < n)
		{
			...
			if (dp->d_type == DT_DIR) {
				...
				if (exp_idx != 0 && exp_idx != ide[id_idx-1].num) {
					std::cout << "error !!! \n";
					perror("Not match .");
					exit(1);
				}
		...
		while (index < n)
		{
			...
			if (dp->d_type == DT_DIR)
				...
				if (dp->d_name[len - 1] == 'd' && dp->d_name[len - 2] == '3' && dp->d_name[len - 3] == 'd') {
					...
					flag = 1;
					std::string p = path + "/" + dp->d_name;
					save_result_one(ide, id_idx, exp_idx, p.substr(0, p.find(".land")) + ".3dlv");
					exp_idx++;
				}
			}
			free(namelist[index]);
...
void load_land_corr(Eigen::VectorXi &land_cor) {
	puts("loading 3d landmarks correspondence...");
	FILE *fp;
	fopen_s(&fp, "./inner_jaw/fan3d_land.txt", "r");
	for (int i = 0; i < G_land_num; i++) fscanf_s(fp, "%d", &land_cor(i));
	//std::cout << cor <<"------------------------------\n"<< '\n';
	fclose(fp);
}
void load_3d_land(std::string p, iden *ide, int id_idx) {
	...
	FILE *fp;
	fopen_s(&fp, p.c_str(), "r");
	ide[id_idx].land_3d.conservativeResize(G_land_num*(ide[id_idx].num + 1), 3);
	for (int i = G_land_num * (ide[id_idx].num); i < G_land_num*(ide[id_idx].num + 1); i++)
		fscanf_s(fp, "%f%f%f", &ide[id_idx].land_3d(i, 0), &ide[id_idx].land_3d(i, 1), &ide[id_idx].land_3d(i, 2));
	fclose(fp);
}